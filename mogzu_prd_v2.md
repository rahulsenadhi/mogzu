...
**Stack**: React · Supabase · Tailwind · N8N · VAPI  
...
- **Phase 1 — Core Platform**: 47 user stories covering all roles and all missing flows (from `mogzu_user_stories.md`)
- **Phase 2 — Add-On Features**: 10 advanced features to be built after Phase 1 foundation is stable (from `Mogzu_AddOn_Features_PRD.md`)
...
> **Status**: Active development. Stories ordered P0 → P1 → P2.  
> **Prerequisite for all P0 stories**: Migrate all localStorage stores to Supabase + implement Supabase RLS per role.
...
> **Status**: NOT IN CURRENT SPRINT. Do not build until Phase 1 core platform is stable and in production.  
> **Prerequisite for all Phase 2 features**: Phase 1 P0 stories complete, Supabase migration done, payment gateway live.  
> **Process before building any feature**: Run codebase audit to check if it already exists (Existing / Partial / Missing).

---

## Add-On Feature 1 — Quick Share Catalogue (Off-Platform Sales)

> **Phase**: Phase 2 — Add-On  
> **Module**: Admin Dashboard  
> **Priority**: High (build first among Phase 2 features)  
> **Requires**: Supabase auth · corporate accounts table · payment gateway (Story 6.1) · vendor catalogue in Supabase · email/WhatsApp notification service  
> **Codebase audit**: Search for `quickshare`, `share-link`, `catalogue-share` before building

### Overview
Close quick orders from non-registered, walk-in, or one-time corporate clients without requiring them to sign up on the platform.
...
### Business Logic
- Link has a configurable expiry (default: 48 hours)
- Disabled items are hidden from client view (not shown as unavailable)
- Submitted selections trigger notification to admin (in-app + email/WhatsApp)
- Payment link integrates with existing payment gateway (UPI / card / wallet)

---

## Add-On Feature 2 — Live Status Tracker with Proof of Execution

> **Phase**: Phase 2 — Add-On  
> **Module**: All Modules (Events, Gifting, DSpace)  
> **Priority**: High  
> **Requires**: Supabase bookings table · Supabase Storage (photo uploads) · OTP service · GPS/geolocation API · notification service (Story 7.2) · booking lifecycle (Stories 3.2, 4.2, 5.2)  
> **Codebase audit**: Search for `status-tracker`, `otp-verify`, `proof-upload` before building

### Overview
Provide tamper-proof, real-time execution tracking for every booking — eliminating disputes and communication gaps.
...
- Admin: Full view + override capability with mandatory reason log

---

## Add-On Feature 3 — Proof of Agreed Conditions & Payments

> **Phase**: Phase 2 — Add-On  
> **Module**: All Modules — Booking Detail Page  
> **Priority**: High  
> **Requires**: Supabase bookings table · payment gateway (Story 6.1) · negotiation/offer price flow · Supabase Storage (document uploads) · PDF export service  
> **Codebase audit**: Search for `agreement`, `payment-milestone`, `booking-proof` before building

### Overview
Maintain a digital paper trail of all commitments — scope, pricing, and payment — for trust and dispute resolution.
...
## Add-On Feature 4 — Gifting Auto Branding Placement Preview

> **Phase**: Phase 2 — Add-On  
> **Module**: Gifting — Order Flow  
> **Priority**: Medium  
> **Requires**: Supabase Storage (logo file storage) · image compositing service (e.g. Cloudinary or custom canvas renderer) · gifting order flow (Stories 4.2, 4.4) · admin approval workflow · vendor communication  
> **Codebase audit**: Search for `branding-preview`, `logo-upload`, `mock-up` before building

### Overview
Allow corporates to preview and approve logo/branding placement on gifting products before production begins.
...
## Add-On Feature 5 — Category & Subcategory Management

> **Phase**: Phase 2 — Add-On  
> **Module**: Admin Dashboard — Catalogue Management  
> **Priority**: Medium  
> **Requires**: Supabase categories table · vendor listings table · admin portal (Story 9.3) · real-time propagation to corporate catalogue (Supabase Realtime)  
> **Codebase audit**: Search for `category-manage`, `enable-category`, `admin-catalogue` before building

### Overview
Admin controls catalogue structure in real time without code deployments.
...
- **Enable / Disable** toggle per category or subcategory — immediately reflected on corporate-facing catalogue
...
## Add-On Feature 6 — Sub-Users & Partner Management (RBAC)

> **Phase**: Phase 2 — Add-On  
> **Module**: Admin Dashboard — Team Management  
> **Priority**: High  
> **Requires**: Supabase auth with role-based claims · Supabase RLS (Phase 1 prerequisite) · admin portal (Story 9.3) · activity logging table · email invite service  
> **Codebase audit**: Search for `sub-user`, `rbac`, `role-permission`, `team-manage` before building

### Overview
Admin builds and manages an internal operations team with controlled, scoped access.
...
## Add-On Feature 7 — Mogzu Own Listings (Mogzu Direct)

> **Phase**: Phase 2 — Add-On  
> **Module**: Admin Dashboard — Listings Management  
> **Priority**: Medium  
> **Requires**: Supabase listings table · vendor listing CRUD flow (Story 3.6) · media upload (Supabase Storage) · corporate-facing catalogue · search/filter system  
> **Codebase audit**: Search for `mogzu-direct`, `own-listing`, `internal-listing` before building

### Overview
Mogzu lists and sells its own proprietary products and services directly on the platform alongside vendor listings.
...
- Publish without vendor approval workflow (admin is both lister and approver)
...
## Add-On Feature 8 — CMS — Website & Landing Page Management

> **Phase**: Phase 2 — Add-On  
> **Module**: Admin Dashboard — Content Management  
> **Priority**: Medium  
> **Requires**: Supabase cms_content table · Supabase Storage (image/banner uploads) · public-facing landing page routes · scheduled publish queue  
> **Codebase audit**: Search for `cms`, `content-manage`, `homepage-edit` before building

### Overview
Admin updates public-facing website content without developer involvement.
...
## Add-On Feature 9 — AI Agents Management Page

> **Phase**: Phase 2 — Add-On (post-MVP phase)  
> **Module**: Admin Dashboard — AI & Automation  
> **Priority**: Medium  
> **Requires**: VAPI or equivalent conversational AI SDK · WhatsApp Business API · Telegram Bot API · web chat widget · knowledge base storage in Supabase · N8N for lead hand-off automation (Story 7.3) · admin portal  
> **Codebase audit**: Search for `ai-agent`, `bot-manage`, `agent-config` before building

### Overview
Configure and monitor Mogzu's AI agents across all communication channels.
...
- Shares relevant catalogue options (links to Quick Share Catalogue — Add-On Feature 1)
...
## Add-On Feature 10 — Database Migration & Scalability Plan

> **Phase**: Phase 2 — Add-On (engineering architecture; no UI required)  
> **Module**: Technical Architecture  
> **Priority**: Post-MVP  
> **Requires**: All Phase 1 data abstraction layers in place before migration  
> **Codebase audit**: N/A — architecture decision; check if abstraction layers (useAuth, storageService, db service, realtimeService) exist before planning migration

### Overview
Engineering architecture decisions to ensure Mogzu can migrate off Supabase with minimal effort when scale requires it.
...
- Maintain all schema changes as portable `.sql` migration files (not Supabase-specific GUI exports)
...
- Route all database queries through a `db` service layer — never raw `supabase.from()` calls in UI components
...
- **Without abstraction layers**: 3–6 weeks (high risk of regressions)

---

## Phase 2 Audit Checklist

Run this before building each Phase 2 feature:

| Feature | Search Terms | Status |
|---|---|---|
| Quick Share Catalogue | `quickshare`, `share-link`, `catalogue-share` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| Live Status Tracker | `status-tracker`, `otp-verify`, `proof-upload` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| Proof of Conditions | `agreement`, `payment-milestone`, `booking-proof` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| Branding Preview | `branding-preview`, `logo-upload`, `mock-up` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| Category Management | `category-manage`, `enable-category`, `admin-catalogue` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| Sub-Users / RBAC | `sub-user`, `rbac`, `role-permission`, `team-manage` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| Mogzu Direct Listings | `mogzu-direct`, `own-listing`, `internal-listing` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| CMS / Content Mgmt | `cms`, `content-manage`, `homepage-edit` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| AI Agents Page | `ai-agent`, `bot-manage`, `agent-config` | `[ ] Existing` `[ ] Partial` `[ ] Missing` |
| DB Migration Plan | N/A — abstraction layers check | `[ ] useAuth hook` `[ ] storageService` `[ ] db service` `[ ] realtimeService` |

---

*Mogzu PRD v2.0 — May 2026*  
*Sources: mogzu_user_stories.md (Phase 1) · Mogzu_AddOn_Features_PRD.md (Phase 2)*
